#include<bits/stdc++.h>//C(n,m)%p n,m较小,p大 无质数限制
...
struct BruteC{//C(n,m)%p n,m 1e5,p 1e9 无质数限制 precal前缀和范围n,m/模数p //O(n*prid(p))
    ll mod,phi;int siz;vector<ll>facs;vector<vector<ll> >mul;
    ll poww(ll bs,ll x){ll res=1;for(bs%=mod;x;x>>=1,(bs*=bs)%=mod)if(x&1)(res*=bs)%=mod;return res;}//when p<1e9
    ll invv(ll bs){return poww(bs,phi-1);}    
    vector<ll>era(vector<ll>& v){vector<ll>v2;for(auto k:v)if(v2.empty()||v2.back()!=k)v2.push_back(k);return v2;}
    void precal(ll _mod,int N){//P:
        mod=_mod;facs=mil.disolve(mod);facs=era(facs);siz=facs.size();
        phi=mod;for(auto k:facs)phi=phi/k*(k-1);
        vector<ll>bas(siz+1,0);bas[0]=1;mul.resize(N+1,bas);
        for(int i=1;i<=N;i++){int x=i;mul[i]=mul[i-1];
            for(int j=1;j<=siz;j++)while(x%facs[j-1]==0)
                x/=facs[j-1],mul[i][j]++;(mul[i][0]*=x)%=mod;}}
    ll C(ll n,ll m){
        if(n<m)return 0;ll res=mul[n][0]*invv(mul[m][0])%mod*invv(mul[n-m][0])%mod;
        for(int i=1;i<=siz;i++)(res*=poww(facs[i-1],mul[n][i]-mul[m][i]-mul[n-m][i]))%=mod;return res;}
}Bruc;
int main(){Bruc.precal(1e9,1e5);cout<<Bruc.C(4,2)<<'\n';}
